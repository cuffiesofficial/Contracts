...
contract CUFFIES is Context, IBEP20, Ownable {
    ...
    function sendETHToFee(address from, address to,uint256 amount) private {
        ...
        _marketingFunds.transfer(amount.mul(3).div(7));
        _operationAddress.transfer(amount.mul(2).div(7));
        }
        _appAddress.transfer(amount.mul(3).div(10));
        _marketingFunds.transfer(amount.mul(2).div(10));
        _operationAddress.transfer(amount.mul(2).div(10));
    }
    ...
    function manualsend(address from, address to) external onlyOwner() {
        uint256 contractETHBalance = address(this).balance;
