...
contract CUFFIES is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "CUFFIES";
    string private constant _symbol = "$CUFFIES" ;
    ...
    constructor(address payable addr1, address payable addr2, address payable addr3, address payable addr4) {
        require(addr1 != address(0), "_appAddress is a zero address");
    ...
    function excludewalletfee(address walletAddress) external onlyOwner() {
       ...
       emit WalletExcludedfromFee(walletAddress);
    }

    function includeWalletFee(address walletAddress) external onlyOwner() {
    ...
    function addmodelwallet(address walletAddress) public onlyOwner() {
       model[walletAddress] = true;
       emit ModelWalletAdded (walletAddress);
    }
    
    function excludemodelwallet(address walletAddress) public onlyOwner() {
       model[walletAddress] = false;
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _companyFee == 0) return;
        _taxFee = 0;
        _companyFee = 0;
   ...
   function setMaxWalletpercent (uint8 maxWalletPercent) external onlyOwner() {
    ...
    function sendETHToFee(uint256 amount) private {
        sendValue(_appAddress,amount.mul(2).div(7));
        sendValue(_marketingFunds,amount.mul(2).div(7));
        sendValue(_operationAddress,amount.mul(2).div(7));
        sendValue(_liquidityWallet,amount.mul(1).div(7));
    }
    
    function openTrading() public onlyOwner {
        require(liquidityAdded,"Liquidity is not added.");
        tradingOpen = true;
    ...
    function addLiquidity() external onlyOwner() {
        ...
        swapEnabled = true;
        liquidityAdded = true;
        IBEP20(pcsV2Pair).increaseAllowance(address(pcsV2Router),type(uint256).max);
        emit LiquidityAdded(true);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tCompany) = _getTValues(tAmount, _taxFee, _companyFee);
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tCompany, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tCompany);
    }

    function _getTValues(uint256 tAmount, uint256 taxFee, uint256 companyFee) private pure returns (uint256, uint256, uint256) {
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tCompany, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
