...
contract CUFFIES is Context, IBEP20, Ownable {
    ...
    function addmodelwallet(address walletAddress) public {
   ...
   function setMaxWalletPermille (uint8 maxWalletPermille) external onlyOwner() {
        require (maxWalletPermille >= 1 && maxWalletPermille <= 1000, "CUFFIES: The max wallet percentage must be between 0.5% (5) and 100% (1000)");
    ...
    function sendETHToFee(uint256 amount) private {
        _appAddress.transfer(amount.mul(2).div(7));
        _marketingFunds.transfer(amount.mul(2).div(7));
        _operationAddress.transfer(amount.mul(2).div(7));
        _liquidityWallet.transfer(amount.mul(1).div(7));
    }
